...
def hlt_error(line, test_no):
    ...
    if(test_no==1):
        ...
        print("[ERROR] Program has more than one halt instruction at the line no."+line+" \nUse only one hlt instruction which is to be at the end of the program.")   

def op_error(line, test_no, op):
    # 1 = not a instr
    if(test_no==1):
        print("[ERROR] "+op+" is not a instruction at the line no. "+ line)

def improper_len_instr(line, test_no, type_instr):
    # test_no = instr_name 
    # type_instr = instr type
    print("[ERROR] "+test_no+" is a type-"+type_instr+" instruction at the line no. "+ line)

def invalid_reg(line, name):
    # an invalid reg name
    print("[ERROR] "+name+" is not a register at the line no. "+ line)

def invalid_imm(line, imm):
    # an invalid imm
    print("[ERROR] "+imm+" is not a valid immediate value at the line no. "+ line +" \nImmediate value must be an integer between 0 and 255 (both including).")

def invalid_mem_addr(line, mem_addr, test_no):
    # an invalid var(0)/label(1)
    if(test_no==0):
        print("[ERROR] "+mem_addr+" is not a valid variable name at the line no. "+ line +" \nVariable name must be initialized first before using it.")
    else:
        print("[ERROR] "+mem_addr+" is not a valid label name at the line no. "+ line +" \nLabel name must be defined first before using it.")

def flags_invalid(line):
    print("[ERROR] Invalid use of FLAGS register at the line no. "+line)